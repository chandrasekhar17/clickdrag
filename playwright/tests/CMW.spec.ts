...
test.describe('Image Labeling', () => {
    ...
    test('Label which is dropped disabled and distractor which is not dropped enabled', async ({ page }) => {
        ...
        await page.locator('select[name="mode_select"]').selectOption('sample');
        await ezt_po.LoadAPI();
        for (let i = 0; i <= 5; i++) {
            if (i <= 3) {
    ...
    test('The Notes icon next to each label should be shown in Check my work mode.', async ({ page }) => {
        let ezt_po = new EZT_PO(page);
        await ezt_po.InputState(records[1].states);
        await page.locator('select[name="mode_select"]').selectOption('test');
        await ezt_po.LoadAPI();
        ...
        await page.locator('select[name="mode_select"]').selectOption('sample');
        await ezt_po.LoadAPI();
        const Notes = await frame.locator('i.dpg-icon-notepad').nth(1);
    ...
    test('Drop zones that are left empty by students must NOT be marked as Correct or Incorrect ', async ({ page }) => {
        let ezt_po = new EZT_PO(page);
        await ezt_po.InputState(records[1].states);
        await page.locator('select[name="mode_select"]').selectOption('test');
        await ezt_po.LoadAPI();
        ...
        await page.locator('select[name="mode_select"]').selectOption('sample');
        await ezt_po.LoadAPI();
        await expect(frame.locator('span.feedback-text')).toHaveCount(3)
    ...
    test('Image Labeling-Each Label instance If any Label has more than one drop zone in Authoring mode, the count of the Label must be the same as the number of drop zones. ', async ({ page }) => {
        let ezt_po = new EZT_PO(page);
        await ezt_po.InputState(records[3].states);
        await page.locator('select[name="mode_select"]').selectOption('test');
        await ezt_po.LoadAPI();
        ...
        await page.locator('select[name="mode_select"]').selectOption('sample');
        await ezt_po.LoadAPI();
        await expect(frame.locator('li.label-item')).toHaveCount(6)
        await expect(frame.locator('.drop-zone.dropable')).toHaveCount(5)
    ...
    test('Validating Notes content and format', async ({ page }) => {
        let ezt_po = new EZT_PO(page);
        ...
        for (let i = 0; i <= 4; i++) {
            if (i === 0) {
                await frame.locator('i.dpg-icon-notepad').nth(i).click();
...
test.describe('Grouping', () => {
    ...
    test('Grouping-The Notes icon next to each label should be shown in Check my work mode.', async ({ page }) => {
        let ezt_po = new EZT_PO(page);
        await ezt_po.InputState(records[4].states);
        await page.locator('select[name="mode_select"]').selectOption('test');
        await ezt_po.LoadAPI();
    ...
    test('Grouping-Validating Notes content and differnt format', async ({ page }) => {
        let ezt_po = new EZT_PO(page);
