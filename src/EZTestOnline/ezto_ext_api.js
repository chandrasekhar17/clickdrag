...
var ez_mediabase = '';
var ez_language = ''; // baseURL from EZTO
...
var EZ = {
  ...
  getParentVarByID: function (varname) {
    ...
    try {
      if (parent.ex_get_varByID) result = parent.ex_get_varByID(varname);
      else this.log('EZ.getParentVarByID() - no ex_get_varByID in parent');
    } catch (err) {
  ...
  getParentVarByName: function (name) {
    ...
    try {
      if (parent.ex_get_varByName) result = parent.ex_get_varByID(name);
      else this.log('EZ.getParentVarByName() - no ex_get_varByName in parent');
    } catch (err) {
  ...
  getParentDivByID: function (name) {
    ...
    try {
      if (parent.ex_get_varByDivID) result = parent.ex_get_varByDivID(name);
      else this.log('EZ.getParentDivByID() - no ex_get_varByDivID in parent');
    } catch (err) {
  ...
  init: function () {
    ...
    if (this.id != '') {
      ...
      this.language = this.getParentVarByID(this.id + '_language');
      var state = '';
  ...
  populateToolStateJson: function () {
    try {
      ...
      toolState.language = this.language;
  ...
  resize: function (width, height) {
    ...
    /*
		...
		*/
    try {
      if (parent.ex_resize) result = parent.ex_resize(this.id, width, height);
      else this.log('EZ.resize() - no ex_resize in parent');
    } catch (err) {
    ...
    try {
      ...
      if (sourceMedia != '') {
        ...
        if (mediaArray.length > 1) {
          ...
          for (i = 1; i < mediaArray.length; i++) this.mediaUrls.push(mediaArray[i]);
        }
      }
    } catch (e) {
        ...
        if (!previousCallTime || elapsedTime > this.toolsavethrottle) {
          ...
          if (this.toolStateJSON.state !== currentState || this.toolStateJSON.completion !== completion) {
            ...
            $.ajax({
              ...
              error: function (xhr) {
                alert(
                  'There was an error saving this question. Reopen assignment to continue.\nSupport link: http://mpss.mhhe.com/contact.php'
                );
  ...
  contactPublisher: function (message) {
    ...
    try {
      if (parent.ex_trigger_contact) parent.ex_trigger_contact(this.mode, this.qid, message);
      else this.log('EZ.contactPublisher() - no ex_trigger_contact in parent');
    } catch (err) {
